...
uint32_t init_tcp_ip_interface()
{
  ...
  // TODO: get interface name from command line
  memcpy(ifr.ifr_name, "wlp2s0", IFNAMSIZ - 1);
  ioctl(raw_socket, SIOCGIFADDR, &ifr);
...
int send_packets_to_ip(uint32_t foreign_ip, char *send_buffer, int send_buff_size)
{
  ...
  servaddr.sin_addr.s_addr = foreign_ip;
  if (connect(raw_socket, (SA *)&servaddr, sizeof(servaddr)) < 0)
  {
    perror("raw socket connection failed to foreign ip");
    return -1;
  }

  // send the packet
  printf("sending packet to %x\n", servaddr.sin_addr.s_addr);
  int size_wrote;
